...
How language flows through the gateway, from request validation to FastAPI dispatch to client echo. Language is a core part of the request and persistence path; this doc records the contract so future contributors do not have to re-derive it from the code.
...
The set is intentionally narrow. Adding a language is a deliberate change: touching this enum, adding a FastAPI engine, and adding a default rhyme mode mapping, not flipping an environment-variable toggle.
...
The presenter echoes the resolved language and mode even when no rhymes were returned, so the client never has to remember what it asked for. If FastAPI's response includes its own resolved mode in `meta.mode`, the presenter prefers that, which is useful when FastAPI made a defaulting decision the gateway didn't.
...
- **No automatic language detection.** Mixed-language line analysis is explicitly out of scope. The gateway will not guess: if the client wants a Spanish rhyme for a Spanish line in an English draft, the client sends `language: 'es'` on the analyze call.
...
4. The DTO validators, defaulting mapper, and presenter pick it up automatically, with no controller changes.
